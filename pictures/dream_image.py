...
fileName, layer, num_iterations, num_repeats  = args.filename, args.layer, args.iter, args.repeats
print(args) 
...
img_result = recursive_optimize(layer_tensor=layer_tensor, image=img_result,
                 ...
                 num_repeats=num_repeats, blend=0.2)

img_result = np.clip(img_result, 0.0, 255.0)
img_result = img_result.astype(np.uint8)
result = PIL.Image.fromarray(img_result, mode='RGB')
result.save(os.path.splitext(fileName)[0]+"_layer%i-iter%i-rep%i.jpg" % (layer,num_iterations,num_repeats))
result.show()
